 ...
 * Admin enrichment: update meta + pricing fields, write to disk, reload transporter.
...
router.put('/transporters/:id/enrich', async (req, res) => {
  try {
    ...
    // Verify transporter exists in service
    const transporter = utsfService.getTransporterById(id);
    if (!transporter) {
    ...
    if (!fs.existsSync(filePath)) {
      return res.status(404).json({ success: false, message: 'UTSF file not found on disk' });
    }

    const utsfData = JSON.parse(fs.readFileSync(filePath, 'utf8'));

    // ── Merge meta fields (safe whitelist) ──
    const metaWhitelist = ['companyName', 'transporterType', 'rating', 'isVerified'];
    for (const key of metaWhitelist) {
      if (meta[key] !== undefined) utsfData.meta[key] = meta[key];
    }

    // ── Merge priceRate fields ──
    ...
    // ── Write to disk ──
    fs.writeFileSync(filePath, JSON.stringify(utsfData, null, 2), 'utf8');
    console.log(`[UTSF ENRICH] Wrote enriched data for ${id}`);

    // ── Reload transporter in memory ──
    utsfService.reloadTransporter(id);
